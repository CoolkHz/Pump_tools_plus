...
class TradeHandler {
  ...
  // 创建交易捆绑包
  async createBundledTransactions(tokenMetadata, mintAddress, minAmount, maxAmount) {
    try {
      // 确保已初始化钱包
      if (!this.devKeypair) {
        throw new Error('未找到Dev钱包')
      ...
      const bundleTxs = [
        ...
        ...this.signerKeyPairs.slice(0, baseWalletCount).map(keypair => {
          const randomAmount = this.getRandomAmount(minAmount, maxAmount)
          this.addLog(`钱包 ${keypair.publicKey.toString().slice(0, 8)}... 随机买入金额: ${randomAmount} SOL`, 'info')
  ...
  // 处理完整流程
  async prepareMetadata(metadata, strategy, contractAddress, minAmount, maxAmount) {
    try {
      // 1. 初始化钱包
